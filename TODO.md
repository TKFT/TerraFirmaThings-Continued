### Bugs and Improvements
- [ ] Implement a FluidLogging system to water log blocks with river water
- [ ] Better fishing net capture mechanic, so it's more involved than just putting a fish in an inventory. Give it a more TFC feel and harder
- [ ] Higher quality gem models for the gem display.
- [ ] Higher quality sling stone entity models and textures.
- [ ] Balance items and make recipe changes.
- [ ] Better crampon texture and armor model.
- [ ] Patchouli HTML export for the online field guide
- [ ] Grindstone wheel Create support, to make it a native Create energy consumer.
- [ ] Tweak sling damage and force values. 
- [ ] Move Hook Javelin model to JSON model

### New Features to add

#### Log Minecraft/Rock Minecart
- [ ] Minecart that functions like a flat bed train car that holds logs.
- [ ] Minecart that functions like a traditional minecart holding rock and ore

#### Horse Shoes
- [ ] Provide a small speed boost for horses
- [ ] Makes them immune to slowness in snow and long grass

#### Shellfish/Crab Trap
- [ ] Produces shellfish or crabs in salt water if they are able to spawn in that area by spawning them in a trapping them.

#### Pickling Jars
- [ ] Function like the jars in TFC Homestead
- [ ] Allow food to be stored in jars of vinegar to be preserved.

#### Other food storage stuff
- [ ] Other cellars, deep freeze, canning/jaring more items, etc?????